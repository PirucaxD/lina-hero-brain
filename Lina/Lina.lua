...
LOG:info("Lina brain v0.5.67 (Phase 4 slice 2): catalog 13 -> 22. User: 'Keep expanding catalog (slice 2: channels + instant disables)'. **9 new entries** in lib/threat_data.lua THREAT_ARRIVAL_TIMING: **6 channel_at_caster** (caster-side modifier names matching the WD Death Ward precedent): modifier_bane_fiends_grip (cp 0.3 caster), modifier_pugna_life_drain (cp 0.3 caster), modifier_pudge_dismember (cp 0.3 caster), modifier_crystal_maiden_freezing_field (cp 0.3 caster), modifier_enigma_black_hole (cp 0.45 caster), modifier_disruptor_static_storm_thinker (cp 0.3 caster, thinker-based but caster pos = thinker pos at cast time). **3 cast_point_targeted** fast disables: modifier_lion_voodoo (Lion Hex, cp 0.3 self), modifier_bane_nightmare (cp 0.3 self), modifier_faceless_void_chronosphere (cp 0.4 caster -- AoE-at-ground centered on FV cast pos; approximation since the kind taxonomy lacks a cast_point_aoe variant, same workaround as Sand King Epicenter in v0.5.66). **Cast point values from common Dota knowledge**; consumers are not yet wired (no per-mod pre-fire timing on save items), so precise values can be tuned when consumers come online. **Channel modifiers use caster-side names** (matching WD Death Ward) because compute_arrival_time asks about the channel itself; brains needing to react to the target-side modifier can translate via separate mapping. **Behavioural impact on Lina**: still minimal-to-none (consumer gap unchanged from v0.5.66). W catalog gate only fires for homing kinds; Pike/Force/EUL/WW dont read compute_arrival_time. The expansion continues groundwork for future consumers (e.g. Lotus/BKB defer their fire until impact_t approaches a threshold) and for uczone-toolkit publish. **Files**: lib/threat_data.lua +~120 lines (9 new entries). Lina.lua banner only. lib/escape.lua + lib/defense.lua + Sniper.lua unchanged from v0.5.61 / v6.15.278. **Verification on next demo**: no new tlogs fire because no consumer is wired. Existing W gate / Pike / Force / EUL / WW behaviour unchanged. **Next slices queued**: more homing projectiles + AoE-at-ground (Earthshaker Fissure / Echo Slam, Magnus Skewer / RP, Sven Storm Bolt, Slardar Crush, Lich Chain Frost, Lina Light Strike Array, etc.) then a consumer wire.")
