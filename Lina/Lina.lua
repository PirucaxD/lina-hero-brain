...
LOG:info("Lina brain v0.5.66 (Phase 4 slice 1): catalog expansion 7 -> 13. User: 'cross check ... and move on to next phase'. **Audit findings**: v0.5.65 verified clean: luac OK, 3 helper-caller sites (WW + Force + Pike, EUL absent as designed), no stale state refs, no Lua errors since v0.5.65 load at L89. WW behavior verified in current log (2 fires post-load): ww_post_move x=-2670 y=646 movable=y -> ww_post_move_fired -> ww_post_move_arrived reissues=14 (1.4s during the 2.5s airborne, well within window). Same shape on the second fire. EUL post-move tlogs correctly absent. Pike self-cast + Force have not triggered in any demo post-v0.5.61 lib/escape fix but structurally rely on the same code path WW uses, so high confidence. **Phase 4 slice 1**: catalog 6 new cast_point_targeted entries that mirror the existing Lion/Sniper/Lina shape: modifier_doom_bringer_doom (cp 0.5), modifier_ice_blast (AA, cp 0.5), modifier_obsidian_destroyer_sanity_eclipse (cp 1.7), modifier_tinker_laser (cp 0.45), modifier_zuus_thundergods_wrath (cp 0.6), modifier_sand_king_epicenter (cp 2.0, impact_pos=caster). All 6 were already in CAST_POINT_THREATS with cp_default values; now duplicated into THREAT_ARRIVAL_TIMING so compute_arrival_time has a catalog hit for them too. **v0.5.56 sync block** auto-applies -- the 6 cp_default values are now derived from catalog at load time, completing the single-source-of-truth migration. The 'absent from catalog keep their literal' carve-out is gone from the comment. **Behavioural impact on Lina**: minimal-to-none. W's catalog gate only fires for homing_charge / homing_carry kinds (Bara, Tusk). Other consumers (Pike, Force, EUL, WW) do not read compute_arrival_time. The catalog expansion is groundwork for future consumers (e.g. category-aware pre-fire timing on Lotus/BKB/Glimmer per impact_t) and for uczone-toolkit publish. **Files**: lib/threat_data.lua +~80 lines (6 entries + comment block update). Lina.lua banner only. lib/escape.lua + lib/defense.lua + Sniper.lua unchanged from v0.5.61 / v6.15.278. **Verification on next demo**: compute_arrival_time hits for the new 6 mods would log via existing tlogs only IF a consumer is wired -- currently none for cast_point_targeted. The dedup effect: cast_point_threat_armed cp= values for Doom/AA/OD/Tinker/Zeus/SK now match catalog (identical to v0.5.65 since values were already in sync, just routed through one source). No regression expected. **Next slices (queued)**: more THREAT_CATEGORY mods (channels, instant disables, homing), then potentially a consumer wire that uses the broader catalog for pre-fire timing decisions.")
