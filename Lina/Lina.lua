...
LOG:info("Lina brain v0.5.68 (Phase 4 slice 3): catalog 22 -> 32 (homing projectiles + AoE-at-ground). User: continue slice 3 while parallel Sniper audit runs. **10 new entries** in lib/threat_data.lua THREAT_ARRIVAL_TIMING covering common stuns / nukes / displacements: modifier_earthshaker_fissure_stun (cp 0.46 self, line stun), modifier_earthshaker_echo_slam (cp 0.5 caster, PBAoE around ES), modifier_earthshaker_earthsplitter (cp 0.5 + post_cast_delay 3.0 caster, long-line delayed AoE -- first use of meaningful post_cast_delay), modifier_magnataur_skewer (cp 0.3 self, line displacement), modifier_magnataur_reverse_polarity_stun (cp 0.55 caster, PBAoE pull stun), modifier_sven_storm_bolt (cp 0.5 self, homing projectile), modifier_slardar_slithereen_crush (cp 0.3 caster, PBAoE), modifier_lich_chain_frost (cp 0.45 self, bouncing projectile), modifier_lina_light_strike_array (cp 0.55 + post_cast_delay 0.5 self, delayed AoE-at-ground), modifier_tiny_avalanche (cp 0.3 self, line/AoE DoT). **All cast_point_targeted** because the kind taxonomy still lacks a dedicated projectile_homing or cast_point_aoe variant; projectile travel is modeled on-its-own-dispatch (mirrors AA Ice Blast precedent from v0.5.66). **Cast point + modifier names** follow canonical Dota patterns but may need VPK-grep verification (see reference_dota_modifier_names_vpk_grep memory) when a consumer comes online. **Behavioural impact on Lina**: still minimal-to-none. W catalog gate consumes only homing_charge / homing_carry. Pike / Force / EUL / WW .fire bodies do not read compute_arrival_time. The catalog now covers 32 entries and most of the major teamfight-relevant threats. **Files**: lib/threat_data.lua +~150 lines (10 entries). Lina.lua banner only. lib/escape.lua + lib/defense.lua + Sniper.lua unchanged from v0.5.61 / v6.15.278. **Parallel Sniper audit** running in background to verify nothing got dropped from Sniper or lib during the v6.15.278 lib/escape extraction (user mentioned a 217-skills reference). Audit covers full diff vs origin/main + table counts in Sniper.lua + lib. **Next**: depending on audit results either continue slice 4 (more projectile-style nukes + Pudge Hook + Mirana Arrow + Skywrath MF) or pivot to wiring a consumer that USES the 32-entry catalog for pre-fire timing decisions.")
